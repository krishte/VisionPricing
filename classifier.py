...
object_indices2 = {0: "Headphones", 1: "Kettle", 2: "Washing machine", 3: "Printer", 4: "Toaster", 5: "Dishwasher", 6: "Blender"}
...
object_model2 = YOLO('oxford_group_project/object_detector/weights/best.pt')
...
object_results2 = object_model2(image_path)
#object_frame = object_results[0].plot()



classes_found = list(object_results[0].boxes.cls.cpu())
classes_found2 = list(object_results2[0].boxes.cls.cpu())
...
def add_object(classes_found, index, item, object_results, object_frame):
    ...
    for box, prob in zip(nonintersecting_boxes, nonintersecting_probs):
        ...
        object_frame = cv2.rectangle(object_frame, (int(box[0]), int(box[3])), (int(box[0]+400), int(box[3])-30) , (255, 0, 0), -1)
        # Object label
        ...
        for brand_index in list(brand_results[0].boxes.cls):
            ...
            if np.array(brand_results[0].boxes.conf)[0] < 0.5:
                continue
            ...
            object_frame = cv2.rectangle(object_frame, (int(box[0]), int(box[3])-30), (int(box[0]+400), int(box[3])-60) , (17, 120, 10), -1)
            object_frame = cv2.putText(object_frame, brand_indices[int(brand_index)] + ": " + format(np.array(brand_results[0].boxes.conf)[0], ".2f")  , (int(box[0]), int(box[3])-35), cv2.FONT_HERSHEY_SIMPLEX ,  1, (255, 255, 255), 3) 
        ...
        desc = product_description['other']
        if item.lower() in desc and any([(x in desc[:desc.index(item.lower())].split(' ')) for x in [y[0] for y in rgb_to_text]]):
            product_description['color'] = [x for x in [y[0] for y in rgb_to_text] if x in desc[:desc.index(item.lower())].split(' ')][0]
...
for index, item in object_indices.items():
    ...
    if index in classes_found:
        add_object(classes_found, index, item, object_results, object_frame)

for index, item in object_indices2.items():
    if index in classes_found2:
        add_object(classes_found2, index, item, object_results2, object_frame)
